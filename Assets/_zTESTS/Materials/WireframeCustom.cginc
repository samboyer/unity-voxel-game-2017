...
struct UCLAGL_v2g
{
	float4  worldPos: WORLDPOS;		// world position
...
float4 _WireColor = {1,1,1,1};	// Color of the line
...
{			
	...
	float4 targetColor = _WireColor * tex2D( _MainTex, input.uv);
	float4 transCol = _WireColor * tex2D( _MainTex, input.uv);
	transCol.a = 0;
